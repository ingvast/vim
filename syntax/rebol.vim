...
syn region	rebolComment	start=+comment+ end=+\]+
...
" Strings
syn region 	matchBraces	start=+{+ end=+{+ contains=matchBraces contained
syn region      rebolString     oneline start=+"+ skip=+^"+ end=+"+ contains=rebolSpecialCharacter
syn region      rebolString     start=+[^#]{+hs=s+1 end=+}+ skip=+{[^}]*}+ contains=rebolSpecialCharacter,rebolString
syn region      rebolString     start=+^{+ end=+}+ skip=+{[^}]*}+ contains=rebolSpecialCharacter,rebolString
" Binary
...
syn keyword     rebolConditional if else unless
...
syn keyword     rebolStatement  remove-each

" Context
syn keyword     rebolStatement  alias bind use

" Object
syn keyword     rebolStatement  import make make-object rebol info? context
...
" Constants
syn keyword     rebolConstant   none pi
